...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        similarity_i2t = torch.matmul(image_feat, text_feat.T) / temperature
        similarity_t2i = similarity_i2t.T

        targets = torch.eye(image_feat.size(0), device=device, dtype=image_feat.dtype)

        loss_i2t = loss_fn(similarity_i2t, targets)
        loss_t2i = loss_fn(similarity_t2i, targets)
