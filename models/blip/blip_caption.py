...
class BlipCaption(BlipBase):
    ...
    def greedy_search(
        ...
        while True:
            ...
            output_logits = self.text_decoder(
                input_ids=input_ids,
                attention_mask=attention_mask,
            ...
            new_ends = (next_tokens != eos_token_id).long().squeeze()
            unfinished_sequences = unfinished_sequences * new_ends
    ...
    def sampling(
        ...
        while True:
            ...
            next_token_logits = enforce_min_length(
                input_ids, next_token_logits, min_length, eos_token_id)
            ...
            topk_logits, topk_indices = torch.topk(next_token_logits, topk, dim=-1)
            topk_probs = F.softmax(topk_logits, dim=-1)
            next_tokens = torch.multinomial(topk_probs, num_samples=1).squeeze(1)
            next_tokens = torch.gather(topk_indices, 1, next_tokens.unsqueeze(-1))
            input_ids = torch.cat([input_ids, next_tokens], dim=1)
